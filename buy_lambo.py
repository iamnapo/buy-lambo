...
def main(key, lock, from_addr, to_addr):
  ...
  chain = NodeProxy(os.getenv('RPC_USER', 'rpcuser'), os.getenv('RPC_PASSWORD', 'rpcpassword')).get_proxy()
  ...
  redeem_script = Script([seq.for_script(), 'OP_CHECKLOCKTIMEVERIFY', 'OP_DROP', 'OP_DUP', 'OP_HASH160',
                          public_key.get_address().to_hash160(), 'OP_EQUALVERIFY', 'OP_CHECKSIG'])
  ...
  fee = max((len(bytes.fromhex(tx.serialize())) * sat_per_byte) / SATOSHIS_PER_BITCOIN, 0)
  if fee == 0:
...
if __name__ == "__main__":
  ...
  main(key = args.priv_key, lock = args.lock, from_addr = args.from_addr, to_addr = args.to_addr)
